#include < errno.h > #include < pigpio.h > #include < nan.h > #include < memory >
...
class GpioISR_t: public GpioCallback_t {
  public: GpioISR_t(): GpioCallback_t() {
...
class GpioAlert_t: public GpioCallback_t {
  public: GpioAlert_t(): GpioCallback_t() {
...
void ThrowPigpioError(int err,
  const char * pigpiocall) {
  char buf[128];
